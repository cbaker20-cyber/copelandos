...
captured idea
  ...
  -> supervised agent works on a branch and opens a draft PR
...
- `GET /api/integrations/control-loop` exposes the read-only map that places vault memory before task prompts and draft PR review.
...
- Store links and original summaries instead of copied copyrighted content.
...
Generated tasks should not include provider keys, private vault contents, or student information.
