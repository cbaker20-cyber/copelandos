...
captured idea
  ...
  -> supervised agent may open a draft PR
...
Recommended capture path pattern:
...
- Do not store secrets, tokens, refresh tokens, OAuth codes, real email content, `.env`, or `.dev.vars` data.
- Do not make the vault public.
- Avoid copied copyrighted content; store links and original summaries instead.
