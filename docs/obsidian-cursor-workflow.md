...
- `config/integrations.json` tracks the future vault/task/draft-PR loop without activating those external systems.
...
The generated task should not include provider keys, private vault contents, student information, or instructions to send email, merge PRs, deploy, delete files, or install arbitrary MCP servers.
