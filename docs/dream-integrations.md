...
1. Keep mobile capture behind the current trusted-origin and token gates (`ALLOWED_ORIGIN`, `API_AUTH_TOKEN`, and capture-only tokens).
2. Add a private vault connector with narrow repository permissions and explicit user review.
...
5. Add live probes only when each connector has tests proving it cannot fake `connected: true`.
