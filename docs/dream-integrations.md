...
- `POST /api/capture/idea` and Shortcut-friendly `GET /api/capture/idea` capture ideas into the CopelandOS inbox.
...
- Do not show a provider, tool, vault, GitHub connector, automation, or external integration as connected without an actual connector/probe.
- Keep CORS restricted to the exact configured `ALLOWED_ORIGIN`; CORS is not authentication.

## Next Build Steps

1. Keep mobile capture behind `API_AUTH_TOKEN` and/or a capture-only `CAPTURE_TOKEN`.
2. Add a private vault connector with narrow repository permissions and explicit tests.
...
5. Add live probes only after each connector has least-privilege credentials, failure tests, and safe error handling.
