...
1. Prefer free-tier and local routes before paid routes when quality is sufficient.
2. Escalate to paid providers only when the task requires it and a configured provider exists.
3. Do not fake live state: `configured` means an env var exists; `connected` requires a future probe.
...
| Free cloud | Groq, Cerebras, Gemini, OpenRouter free models | Fast capture classification, summarization, low-cost coding help | Respect rate limits and provider terms |
| Paid direct | Anthropic, OpenAI | Complex planning, security review, high-quality coding | Budget guard before escalation |
| Gateway | LiteLLM or similar | Unified routing and fallback across owned keys | Gateway config must be explicit |
| OpenRouter fallback | OpenRouter fallback routing | Opportunistic model routing | Do not claim consensus unless multiple model calls actually run |
...
5. Add optional gateway support behind an explicit base URL and allowlist.
