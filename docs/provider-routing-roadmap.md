...
- `connected` remains false for provider statuses unless a future live probe proves connectivity.
...
- OpenRouter separates provider-layer failover from model-level fallbacks. Provider failover can keep one model alive across providers, while model fallback changes model IDs by priority list: <https://openrouter.ai/docs/guides/routing/model-fallbacks>
- LiteLLM documents router retries, fallback groups, cooldowns, and load balancing strategies such as simple shuffle, least busy, usage based, latency based, and cost based routing: <https://docs.litellm.ai/docs/router_architecture>
- MCP safety guidance requires host-level user consent and tool safety controls before invoking arbitrary tools: <https://modelcontextprotocol.io/specification/2025-06-18>
...
| OpenRouter/Fusion | OpenRouter auto/fallback routing | Council-style or opportunistic model routing | Do not claim consensus unless multiple model calls are actually made |
