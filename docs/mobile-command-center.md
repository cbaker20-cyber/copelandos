...
- The dashboard is shaped for small screens and works as a PWA-style page.
- `POST /api/capture/idea` accepts `text`, `source`, `tags`, `project`, and `urgency`.
- `GET /api/capture/idea` exists for simple Apple Shortcuts URL actions when capture authorization permits it.
...
If `CAPTURE_TOKEN` is configured, add an `Authorization: Bearer <token>` header or a `token=<token>` query parameter. Do not reuse provider, GitHub, Gmail, or local-agent tokens for mobile capture.
...
- Use a capture-only token for Shortcuts when public mobile access is enabled.
- Add request size limits and rate limits before broad exposure.
- Do not include private student data in captured text.
- The Shortcut must not call endpoints that create drafts, write vault notes, run local actions, merge PRs, deploy, delete files, or send email.
