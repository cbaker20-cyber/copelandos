...
|---|---|---|
| **Public** | `/api/health`, `/api/status`, `/api/projects`, `GET /api/agents`, `GET /api/tasks`, `GET /api/planning-memory`, registry reads, permission rules, brain/orchestration status | Capability and registry metadata only; no secret-backed side effects |
| **Planning mutation** | `POST /api/planning-memory`, `PATCH /api/planning-memory/:id`, `POST .../history`, `decisions`, `dependencies`, `executions` | Create and update planning memory |
