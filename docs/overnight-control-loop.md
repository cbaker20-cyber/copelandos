...
6. Use AI council only as a mock/scaffold until real providers are called and tested
...
`GET /api/integrations/control-loop` exposes this map for the dashboard. It reports implemented internal modules as ready, scaffolded systems as scaffolded, and external systems as not connected unless future live probes exist.
...
- The task requires sending email, merging PRs, deploying, deleting files, arbitrary shell execution, or screen control.
- The implementation would require claiming provider connectivity or live integrations that have not been implemented and tested.
...
- The `/console` dashboard panel displays the loop from Worker data.
