...
- **Integration registry:** read-only roadmap of mobile, vault, provider, task, and reporting integrations. External entries never report connected without a future live probe.
...
| `GET /api/integrations` | Read-only integration roadmap and validation |
| `POST /api/integrations/check` | Fail-closed integration status check |
| `GET /api/integrations/control-loop` | Overnight/mobile control-loop map for the dashboard |
