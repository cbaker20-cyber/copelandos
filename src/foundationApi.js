  ...
  if (path === '/api/status') {
    ...
    return json({
      ...
      modules: {
        ...
        automations: { connected: false, endpoint: '/api/automation/integrations', count: integrations.length, configured: integrations.filter((item) => item.configured).map((item) => item.id) },
