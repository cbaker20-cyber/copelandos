...
export async function handleFoundationRequest({
  ...
  if (path === '/api/status') {
    ...
    return json({
      ...
      modules: {
        ...
        automations: { connected: false, endpoint: '/api/automation/integrations', count: integrations.length, configured: integrations.filter((item) => item.connected).map((item) => item.id), message: 'Automation registry is available, but external automators are not live-probed by status.' },
        integrations: { connected: false, endpoint: '/api/integrations', summary: getIntegrationSummary(env) },
  ...
  if (path === '/api/integrations/check') {
    ...
    if (typeof body.integrationId !== 'string' || !body.integrationId.trim()) {
      return json({ ok: false, error: 'integrationId is required.' }, 400);
    }
    return json(checkIntegration(body.integrationId.trim(), env));
  }
