...
function isInternalImplemented(integration) {
  return integration.mode === 'internal' && integration.status === 'implemented';
}

function publicIntegration(integration, env = {}) {
  const configured = hasRequiredEnv(env, integration.requiredEnvVars);
  const ready = isInternalImplemented(integration);
  ...
  return {
    ...
    honestStatus: ready
      ...
      : configured
        ? 'Required configuration is present, but no live connection probe has run.'
...
export function checkIntegration(id, env = {}) {
  ...
  if (!integration.ready) {
    return {
      ...
      integration,
      connected: false,
      scaffold: integration.status === 'scaffold-only' || integration.stage === 'planned',
      reason: `Integration '${id}' is not active for autonomous execution.`,
    };
  }
...
export function validateIntegrationRegistry() {
  ...
  for (const integration of INTEGRATIONS) {
    ...
    if (!integration.category) errors.push(`${integration.id} missing category.`);
    if (!integration.status) errors.push(`${integration.id} missing status.`);
