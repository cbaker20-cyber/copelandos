...
export function renderCommandCenterHtml({ nonce = '' } = {}) {
  const nonceAttr = nonce ? ` nonce="${htmlAttribute(nonce)}"` : '';
  return `<!doctype html>
...
<body>
  <main class="shell">
    ...
    <section id="system" class="section grid">
      ...
      <article class="panel wide" id="loop"><div class="head"><div><div class="title">Overnight control loop</div><div class="muted">Capture, classify, plan, route, draft PR, then report back. Scaffold only; no automatic execution.</div></div><span class="badge warn" id="loop-badge">loading</span></div><div class="form"><div class="loop-list" id="loop-list"><div class="mini">Loading control loop from the Worker...</div></div></div></article>
  ...
  <script${nonceAttr}>
    ...
    async function loadControlLoop(){
      try{
        ...
        $('loop-list').innerHTML = loop.map((step) => {
          const integration = step.integration || {};
          const state = integration.ready ? 'ready' : (integration.status || 'planned');
          return '<div class="loop-step"><span class="loop-num">' + escapeHtml(step.step) + '</span><div><strong>' + escapeHtml(step.name) + '</strong><small>' + escapeHtml(step.from) + ' -&gt; ' + escapeHtml(step.to) + '</small></div><span class="loop-state">' + escapeHtml(state) + '</span></div>';
        }).join('') || '<div class="mini">No control-loop steps were returned.</div>';
        $('loop-badge').textContent = loop.some((step) => step.integration && step.integration.ready) ? 'partly ready' : 'scaffold';
        log('Control loop loaded', { steps: loop.length, delivery: data.morningReport && data.morningReport.delivery });
      }catch(error){
        $('loop-badge').textContent = 'unavailable';
        $('loop-list').innerHTML = '<div class="mini">Control-loop registry unavailable: ' + escapeHtml(error.message) + '</div>';
