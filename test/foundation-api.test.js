...
test('/api/status does not claim external integrations are connected from env alone', async () => {
  const response = await worker.fetch(new Request('https://worker.example/api/status'), {
...
test('foundation integration control loop returns expected steps', async () => {
  ...
  assert.match(result.architectureText, /copelandos-inbox/);
