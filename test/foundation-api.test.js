...
test('system status reports configured automation env without claiming a connection', async () => {
  const response = await worker.fetch(new Request('https://worker.example/api/status'), { N8N_API_KEY: 'test-key' }, {});
  const result = await response.json();
  assert.equal(response.status, 200);
  assert.equal(result.modules.automations.connected, false);
  assert.deepEqual(result.modules.automations.configured, ['n8n']);
});
