...
test('no integration fakes connected=true from config or env alone', () => {
  const integrations = listIntegrations({
    env: {
      CAPTURE_TOKEN: 'shortcut-token',
      GITHUB_TOKEN: 'github-token',
      GITHUB_REPO: 'owner/vault',
    },
  });

  assert.ok(integrations.some((integration) => integration.configured));
  assert.ok(integrations.every((integration) => integration.connected === false));
});

test('scaffold-only integrations fail closed even when configured', () => {
  const shortcut = getIntegration('iphone-shortcuts', { CAPTURE_TOKEN: 'shortcut-token' });
  const result = checkIntegration('iphone-shortcuts', { CAPTURE_TOKEN: 'shortcut-token' });
...
test('planned integrations fail closed even when credentials are present', () => {
...
test('implemented internal integrations are ready without external connectivity claims', () => {
  const result = checkIntegration('safe-tool-registry', {});

  assert.equal(result.ok, true);
  assert.equal(result.allowed, true);
  assert.equal(result.connected, false);
  assert.equal(result.integration.ready, true);
...
test('control loop maps every step to registered integration status', () => {
  ...
  assert.equal(loop[0].name, 'Capture');
  assert.equal(loop.at(-1).name, 'Report Back');
});
