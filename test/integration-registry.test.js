...
test('integration registry never fakes connected=true', () => {
  ...
  assert.equal(getIntegrationSummary({ GITHUB_TOKEN: 'token', GITHUB_REPO: 'owner/repo' }).connected, 0);
});

test('configured scaffold-only integrations still fail closed', () => {
...
test('implemented internal integrations are ready but not externally connected', () => {
  const inbox = checkIntegration('copelandos-inbox', {});
  assert.equal(inbox.ok, true);
  assert.equal(inbox.allowed, true);
  assert.equal(inbox.connected, false);
  assert.equal(inbox.integration.ready, true);
});
